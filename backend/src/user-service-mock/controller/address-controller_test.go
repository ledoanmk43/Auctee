...
func CreateTestAddress(t *testing.T) (*service.MockIAddressService, *controller.AddressController) {
	ctl := gomock.NewController(t)
	defer ctl.Finish()
	mockSvc := service.NewMockIAddressService(ctl)
	userCtl := controller.NewAddressControllerDefault(mockSvc)
...
func TestAddressController_CreateAddress(t *testing.T) {
	...
	//Mock
	mockSrv.EXPECT().CreateAddress(gomock.Any()).Return(&entity2.Address{
		Model:       gorm.Model{},
		Firstname:   "",
		Lastname:    "",
		Phone:       "",
...
func TestAddressController_GetAddress(t *testing.T) {
	...
	mockSvr.EXPECT().GetAddress(gomock.Any()).Return(&[]entity2.Address{{
		Model:       gorm.Model{},
		Firstname:   "",
		Lastname:    "",
		Phone:       "",
...
func TestAddressController_UpdateAddress(t *testing.T) {
	mockSvc, addressCtl := CreateTestAddress(t)
	//Mock svc
	mockSvc.EXPECT().UpdateAddress(gomock.Any()).Return(&entity2.Address{
		Model:       gorm.Model{},
		Firstname:   "",
		Lastname:    "",
		Phone:       "",
...
func TestAddressController_GetAddressById(t *testing.T) {
	mockSvr, userCtr := CreateTestAddress(t)
	mockSvr.EXPECT().GetAddressById(gomock.Any()).Return(&entity2.Address{
		Model:       gorm.Model{},
		Firstname:   "",
		Lastname:    "",
		Phone:       "",
